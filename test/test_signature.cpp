...
TEST_CASE("Default, empty signature"
...
TEST_CASE("Construct signature with name and email"
	  * test_suite("signature")) {
...
TEST_CASE("Construct signature from formatted buffer"
	  * test_suite("signature")) {
  std::string buffer{
    "Foo Bar <foo.bar@baz.com> 1580583850756 0600"};
...
TEST_CASE("Clone signature struct"
	  * test_suite("signature")) {
  signature foo("Foo Bar", "foo.bar@baz.com", 50, 3);
  auto bar = foo.copy();
  REQUIRE(bar.name() == "Foo Bar");

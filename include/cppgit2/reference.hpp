...
public:
  reference();
  reference(git_reference * c_ptr = nullptr, ownership owner = ownership::libgit2);
  ~reference();
  ...
  // This will transform the reference name into a name "human-readable" version. If no shortname is
  // appropriate, it will return the full name.
  ...
  // If a direct reference is passed as an argument, a copy of that reference is returned. This copy
  // must be manually freed too.
  reference resolve();
  ...
  //
  // To find the OID of a symbolic ref, call reference.resolve() and then this function (or
  // maybe use reference.name_to_id() to directly resolve a reference name all the way through
  // to an OID).
  oid target() const;

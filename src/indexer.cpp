...
void indexer::append(void *data, size_t size) {
  if (git_indexer_append(c_ptr_, data, size, const_cast<git_indexer_progress *>(progress_.c_ptr_)))
    throw git_exception();
}

void indexer::commit() {
  if (git_indexer_commit(c_ptr_, const_cast<git_indexer_progress *>(progress_.c_ptr_)))
    throw git_exception();
}

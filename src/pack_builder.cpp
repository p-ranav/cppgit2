...
void pack_builder::insert_object(const oid &commit_id, const std::string &name) {
  auto name_c = name == "" ? nullptr : name.c_str();
  if (git_packbuilder_insert(c_ptr_, commit_id.c_ptr(), name_c))
    throw exception();
}

void pack_builder::insert_object_recursively(const oid &commit_id, const std::string &name) {
  auto name_c = name == "" ? nullptr : name.c_str();
...
size_t pack_builder::size() const { return git_packbuilder_object_count(c_ptr_); }

void pack_builder::set_progress_callback(std::function<void(int, uint32_t, uint32_t)> callback) {
  ...
  auto callback_c = [](int stage, uint32_t current, uint32_t total, void *payload) {

...
index::index() : c_ptr_(nullptr), owner_(ownership::libgit2) {
  git_libgit2_init();
}
...
void index::add_entries_that_match(
  ...
  auto callback_c = [](const char *pathspec, const char *matched_pathspec, void *payload) {
...
void index::add_entry_from_buffer(const entry &entry, const std::string &buffer) {
  if (git_index_add_frombuffer(c_ptr_, entry.c_ptr(), buffer.c_str(), buffer.size()))
    throw exception();
}
...
void index::add_conflict_entry(const entry &ancestor_entry, const entry &our_entry,
...
void index::for_each_conflict(
  ...
  int ret;
  while ((ret = git_index_conflict_next(&ancestor_out, &our_out, &their_out, iter)) == 0) {
    entry ancestor_payload(nullptr), our_payload(nullptr), their_payload(nullptr);
...
void index::read(bool force) {
  if (git_index_read(c_ptr_, force)) throw exception();
}

void index::read_tree(const tree &tree) {
  if (git_index_read_tree(c_ptr_, tree.c_ptr())) throw exception();
}

void index::remove_entry(const std::string &path, index::stage stage) {
  if (git_index_remove(c_ptr_, path.c_str(), static_cast<int>(stage))) throw exception();
}
...
void index::remove_entry_by_path(const std::string &path) {
  if (git_index_remove_bypath(c_ptr_, path.c_str())) throw exception();
}

void index::remove_entries_in_directory(const std::string &dir, stage stage) {
  if (git_index_remove_directory(c_ptr_, dir.c_str(), static_cast<int>(stage))) throw exception();
}

void index::set_index_capabilities(capability caps) {
  if (git_index_set_caps(c_ptr_, static_cast<int>(caps))) throw exception();
}

void index::set_version(unsigned int version) {
  if (git_index_set_version(c_ptr_, version)) throw exception();
}
...
void index::write() {
  if (git_index_write(c_ptr_)) throw exception();
}

oid index::write_tree() {
  git_oid result;
  if (git_index_write_tree(&result, c_ptr_)) throw exception();
  return oid(&result);
}

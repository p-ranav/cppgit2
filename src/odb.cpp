...
odb::odb(git_odb *c_ptr, ownership owner) : 
...
odb::backend odb::create_backend_for_loose_objects(const std::string &objects_dir, 
  ...
  odb::backend result;
  if (git_odb_backend_loose(&result.c_ptr_, objects_dir.c_str(), compression_level, do_fsync, dir_mode, static_cast<unsigned int>(mode)))
    throw git_exception();
  return result;
}

odb::backend odb::create_backend_for_one_packfile(const std::string &index_file) {
  odb::backend result;

...
commit::commit(git_commit * c_ptr, ownership owner) : c_ptr_(c_ptr), owner_(owner) {
  git_libgit2_init();    
}
...
void commit::amend(const oid &id, const std::string &update_ref, 
  ...
  const std::string &message_encoding, const std::string &message, const cppgit2::tree &tree) {
  if (git_commit_amend(const_cast<git_oid *>(id.c_ptr()), 
    c_ptr_, update_ref.c_str(),
    author.c_ptr(), committer.c_ptr(), message_encoding.c_str(), message.c_str(),
    tree.c_ptr()))
    throw exception();
}
...
oid commit::parent_id(unsigned int n) const { return oid(git_commit_parent_id(c_ptr_, n)); }

unsigned int commit::parent_count() const { return git_commit_parentcount(c_ptr_); }
...
offset_minutes commit::time_offset() const { return git_commit_time_offset(c_ptr_); }
